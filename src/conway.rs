use crate::{field::Fp, poly::Poly, traits::Zero};
...
pub fn conway_poly_fp<const P: u64>(n: u64) -> Option<Poly<Fp<P>>> {
    conway_poly(P, n).map(|coeffs| {
        Poly::new(coeffs.iter().map(|&c| Fp::<P>::new(c)).collect())
    })
}
...
fn test_conway() {
    ...
    assert_eq!(conway_poly_fp::<3>(3), Some(Poly::new(vec![Fp::<3>::new(1), Fp::<3>::new(2), Fp::<3>::zero() , Fp::<3>::new(1)] )));
}